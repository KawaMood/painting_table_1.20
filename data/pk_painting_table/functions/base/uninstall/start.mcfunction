...
data modify storage pk.common:data Uninstall.Array.Search append from storage pk.pa_ta:data Blocks.PaintingTables[]

# Logs
tellraw @a [{"text": "Started uninstalling process for ","color": "yellow"},{"text": "KawaMood's Painting Table","color": "aqua","bold": true},{"text":". It might take some seconds...","color": "yellow"}]

# Recursive function
execute if data storage pk.common:data Uninstall.Array.Search[{}] run function pk_painting_table:base/uninstall/recursive
execute unless data storage pk.common:data Uninstall.Array.Search[{}] run function pk_painting_table:base/uninstall/stop